...
fn setup_sysinfo(config: &Config) -> System {
...
async fn main() -> anyhow::Result<()> {
    // setup logging
    let init = tracing_subscriber::fmt::init();
    ...
    let mut system = setup_sysinfo(&config);
    ...
    loop {
        tokio::select! {
            ...
            Some(()) = reload_rx.recv() => {
                ...
                hot_reloading::try_reload(args, &config, &mut system, &mut interval);
            }
